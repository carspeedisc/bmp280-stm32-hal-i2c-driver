 ...
 * @param adc_P Raw ADC value from sensor
 * @return Pressure in Pa (as unsigned 32-bit fixed-point Q24.8)
 */
 ...
 * @param osrs_t  Temperature oversampling (BMP280_OSRS_X1 ... X16 or SKIP)
 * @param osrs_p  Pressure oversampling (BMP280_OSRS_X1 ... X16 or SKIP)
                  ...
                  uint8_t t_sb, uint8_t filter)
{
    ...
    if (HAL_I2C_Mem_Write(&hi2c1, BMP280_I2C_ADDR, BMP280_REG_RESET,
                          I2C_MEMADD_SIZE_8BIT, &tmp, 1, 100) != HAL_OK)
    {
        return -1;
    }
    ...
    /* Verify chip ID */
    if (HAL_I2C_Mem_Read(&hi2c1, BMP280_I2C_ADDR, BMP280_REG_CHIPID,
                         I2C_MEMADD_SIZE_8BIT, &tmp, 1, 100) != HAL_OK)
    {
        return -2;
    }
    ...
    if (HAL_I2C_Mem_Write(&hi2c1, BMP280_I2C_ADDR, BMP280_REG_CTRL_MEAS,
                          I2C_MEMADD_SIZE_8BIT, &tmp, 1, 100) != HAL_OK)
    {
        return -5;
    }
    ...
    if (HAL_I2C_Mem_Write(&hi2c1, BMP280_I2C_ADDR, BMP280_REG_CONFIG,
                          I2C_MEMADD_SIZE_8BIT, &tmp, 1, 100) != HAL_OK)
    {
        return -6;
    }
...
int BMP280_ReadSensor(float *temp_c, float *press_hpa)
{
    ...
    do
    {
        if (HAL_I2C_Mem_Read(&hi2c1, BMP280_I2C_ADDR, BMP280_REG_STATUS,
    ...
    if (HAL_I2C_Mem_Read(&hi2c1, BMP280_I2C_ADDR, BMP280_REG_PRESS_MSB,
    ...
    *temp_c    = temp_raw / 100.0f;           // °C
    *press_hpa = press_raw / 256.0f;          // hPa
